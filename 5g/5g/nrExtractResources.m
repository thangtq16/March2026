function varargout = nrExtractResources(ind,varargin)
...
%#codegen
    ...
    for i = 1:ngrids
        ...
        grid = varargin{i}; % Extract grid from array
        
        indPrototype = cast(zeros(0,'like',real(grid([]))), underlyingType(ind)); % Prototype for output indices
        ...
        coder.varsize('linInd',[Inf Inf Inf],[1 1 1]);
        if strcmpi(opts.IndexStyle,'subscript')
            P = max(ind(:,3));
        ...
        if (ndims(grid) < 4 && strcmpi(opts.ExtractionMethod,'direct'))
            ...
            if strcmpi(opts.ExtractionMethod,'allplanes')
                % Get linear indices for a single plane
                linInd = sub2ind([size(grid,1) size(grid,2)],subIndIn(:,1), ...
                                 subIndIn(:,2));

                % Project indices over receive antennas and higher dimensions
                linIndExtended = [linInd linInd+((1:projDims(2)*projDims(1)-1)* ...
                                                 size(grid,1)*size(grid,2))];
                linInd = reshape(linIndExtended,[size(subIndIn,1),projDims(2), ...
                                                 projDims(1)]);

                if strcmpi(opts.IndexStyle,'subscript')
                    % Generate subIndOut only if needed
                    [col1,col2,col3,col4] = ind2sub(size(grid),linInd(:));
                    subIndOut = [col1 col2 col3 col4];
                end
            else
                % Project indices over receive antennas and higher dimensions
                ...
                subR = ind2sub([projDims,1],addedInd(:));
        ...
        % Return indices of same class as input indices using prototype
        out{i+ngrids} = cast(indOut,'like',indPrototype);

    end

    [varargout{1:max(nargout,1)}] = out{1:max(nargout,1)};
