function grid = OFDMDemodulate(waveform,info,nSlot,N,hasSampleRate)
...
%#codegen
    ...
    if (hasSampleRate || isempty(coder.target))
        if (any([r.L r.M]~=1))
            ...
            fftin = resample(waveform,r.M,r.L,h);
        else
            fftin = waveform;
        end
    else
    ...
    grid = ofdm_demodulate(fftin(1:T,:),info.Nfft,cpLengths(1:N(1)),symOffset,nullIndices);
    
    % Phase decompensation, TS 38.211 Section 5.4
    if (any(symbolPhases(1:N(1))~=0))
        grid = grid .* exp(-1i * symbolPhases(1:N(1)));
    end
    
end

function y = ofdm_demodulate(x,nfft,cplen,symoffset,nullidx)

    p = struct();
    p.NumReceiveAntennas = size(x,2);
    p.FFTLength = nfft;
    p.NumSymbols = numel(cplen);
    p.NullIndices = nullidx;
    p.hasPilots = false;
    p.PilotIndices = [];
    p.SymbolOffset = symoffset;
    p.CyclicPrefixLength = cplen;
    y = comm.internal.ofdm.demodulate(x,p);

    dataIdx = true(nfft,1);
    dataIdx(nullidx) = false;
    y = y(dataIdx,:,:);
