function dec = decode(in,F,L,iIL,crcLen,padCRC,rnti,qPC)
%decode Polar decode
...
%#codegen
    ...
    % Bit-wise reverse
    br = bit2int(flipud(int2bit(0:N-1,m)),m);
...
function [sttStr, arrayPtrLLR, arrayPtrC] = initializeDataStructures(N,L,m)
    ...
    maxL = 64;
    coder.varsize('parrayPtrLLR',[maxMplus1 maxL], [1 1]);
    coder.varsize('parrayPtrC',[maxMplus1 maxL], [1 1]);
