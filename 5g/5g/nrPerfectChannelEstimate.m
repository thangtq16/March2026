function H = nrPerfectChannelEstimate(varargin)
...
%#codegen
    ...
    % Parse and validate inputs and derive additional info based on inputs
    if isa(varargin{1},'nrCarrierConfig')
        [pathGains,pathFilters,offset,sampleTimes, ...
            ofdminfo,sampleTimesDefaulted,initialNSlot,hasSampleRate] = ...
            parseCarrierSyntax(varargin{:});
    else
    [pathGains,pathFilters,offset,sampleTimes, ...
        ofdminfo,sampleTimesDefaulted,initialNSlot,hasSampleRate] = ...
            parseFlatSyntax(varargin{:});
    end

    if isa(pathGains,'gpuArray')
        pathFilters = gpuArray(pathFilters);
    elseif isa(pathFilters,'gpuArray')
        pathGains = gpuArray(pathGains);
    end
    ...
    if (sampleTimesDefaulted)
        ...
        if (Ncs > 1)
            coder.internal.errorIf(Ncs < samplesPerSlot, ...
                'nr5g:nrPerfectChannelEstimate:TooFewPathGains', ...
                Ncs,sprintf('%g',samplesPerSlot));
        end
    end
    ...
    % first dimension element for each OFDM symbol start.
    pathGains = pathGains(idx,:,:,:);

    % If SampleRate was specified or the CIR will be truncated due to the
    % fraction of the CP being removed prior to OFDM demodulation,
    % calculate the channel response by OFDM demodulation of the CIR.
    if hasSampleRate || truncatesImpulseResponse(pathFilters,offset,ofdminfo)

        % Permute to put the multipath components in the first dimension
        % and switch the antenna dimensions. The pathGains are now of size
        % Np-by-Nr-by-Nt-by-N
        ...
        % If SampleRate was not specified and there is no CIR truncation,
        % calculate the OFDM response as the superposition of path filters
        % OFDM response weighted by the path gains.
        H = nr5g.internal.OFDMChannelResponse(ofdminfo,pathGains,pathFilters,offset);
...
% Check if the OFDM demodulator would remove non-zero samples of the CIR
function out = truncatesImpulseResponse(pathFilters,offset,ofdminfo)
    cpf = ofdminfo.CyclicPrefixFraction;
    cpl = ofdminfo.CyclicPrefixLengths(2);
    removedSamples = fix(cpl*cpf) - max(cpl-offset,0);
    out = any(pathFilters(1:removedSamples,:),"all");
end

% Carrier syntax parser
function [pathGains,pathFilters,offset,sampleTimes,ofdminfo, ...
            sampleTimesDefaulted,initialNSlot,hasSampleRate] = parseCarrierSyntax(carrier,pathGains,pathFilters,offset,sampleTimes,nvpairs)
    arguments
        carrier (1,1) {mustBeA(carrier,'nrCarrierConfig')}
        pathGains (:,:,:,:) {mustBeFloat}
        pathFilters (:,:) {mustBeUnderlyingType(pathFilters,'double')}
        offset (1,1) {validateTimingOffset(offset,pathFilters)} = nrPerfectTimingEstimate(pathGains,pathFilters);
        sampleTimes (:,1) {validateSampleTimes(sampleTimes,pathGains)} = [];
        nvpairs.Nfft (:,1) {validateNfftOrSampleRate(nvpairs.Nfft,'Nfft')} = [];
        nvpairs.SampleRate (:,1) {validateNfftOrSampleRate(nvpairs.SampleRate,'SampleRate')} = [];
        nvpairs.CyclicPrefixFraction (1,1) {mustBeNumeric,mustBeReal,mustBeInRange(nvpairs.CyclicPrefixFraction,0,1)} = 0.5
    end
    ...
    hasSampleRate = ~isempty(nvpairs.SampleRate);
    coder.internal.errorIf(hasSampleRate && ~coder.target('MATLAB'),'nr5g:nrPerfectChannelEstimate:CompilationCarrierSampleRate')

    ofdminfo = nr5g.internal.OFDMInfo(carrier,nvpairs);
    sampleTimesDefaulted = isempty(sampleTimes);
    if sampleTimesDefaulted
    ...
    [pathFilters,offset] = adjust(pathFilters,offset, ...
        ofdminfo.CyclicPrefixLengths,ofdminfo.Nfft);

    end
    
% Flat syntax parser
function [pathGains,pathFilters,offset,sampleTimes,ofdminfo, ...
            sampleTimesDefaulted,initialNSlot,hasSampleRate] = parseFlatSyntax(pathGains,pathFilters,nrb,scs,initialNSlot,varargin)

    arguments
        pathGains (:,:,:,:) {mustBeFloat}
        pathFilters (:,:) {mustBeUnderlyingType(pathFilters,'double')}
        nrb (1,1) {mustBeNumeric,mustBeInteger,mustBeInRange(nrb,1,275)}
        scs (1,1) {mustBeNumeric,mustBeInteger,mustBeMember(scs,[15 30 60 120 240 480 960])}
        initialNSlot (1,1) {mustBeNumeric,mustBeInteger,mustBeNonnegative}
    end
    arguments (Repeating)
        varargin
    end
    ...
    % Parse the CPL syntax: The CPL parameter can appear at the start,
    % between, or end of the TOFFSET and SAMPLETIMES parameters. If CPL is
    % specified and if there are more than 3 values in varargin or the
    % paramter after CPL is char/string then it will be assumed that the
    % user is trying to specify NV-pairs. If CPL has not been specified
    % then parse the NV pairs.
    lvarargin = length(varargin);
    ecp = false;
    cplSpecified = false;
    numericIdx = [0 0];
    idx = 1;
    cplOpts = {'extended','normal'};
    optNames = {'Nfft','SampleRate','CyclicPrefixFraction','CyclicPrefix'};
    if lvarargin == 0
        nvpairs = nr5g.internal.parseOptions(mfilename,optNames);
    else
        for i = 1:lvarargin
            p = varargin{i};
            if ischar(p) || isstring(p)
                % Second time this branch is entered assume that user is
                % attempting to specify NV pairs
                coder.internal.errorIf(cplSpecified,'nr5g:nrPerfectChannelEstimate:CPValueOnlyOption')
                cplSpecified = any(cellfun(@(x)contains(x,p,'IgnoreCase',true),cplOpts));

                % If the char/string is not constant during code generation
                % then assume user is using CPL syntax
                if ~coder.internal.isConst(p) || cplSpecified
                    cpl = validatestring(p,cplOpts,mfilename,'CPL');
                    ecp = strcmpi(cpl,'extended');
                    % Get defaults excluding 'CyclixPrefix'
                    nvpairs = nr5g.internal.parseOptions(mfilename,{'Nfft','SampleRate','CyclicPrefixFraction'});
                else
                    % Must be start of NV pairs
                    nvpairs = nr5g.internal.parseOptions(mfilename,optNames,varargin{i:end});
                    break
                end

            else % Numeric only path
                if idx>2
                    % Assume that the function was called with an attempt to
                    % specify NV-pairs, but a value was supplied instead of
                    % a name.
                    coder.internal.error('Coder:toolbox:ExpectedParameterName');
                elseif i == lvarargin && ~cplSpecified
                    % Get defaults including CyclicPrefix
                    nvpairs = nr5g.internal.parseOptions(mfilename,optNames);
                end
                numericIdx(idx) = i;
                idx = idx + 1;
    ...
    if isfield(nvpairs,'CyclicPrefix')
        ecp = strcmpi(nvpairs.CyclicPrefix,'extended');
    end
    ...
    hasSampleRate = ~isempty(nvpairs.SampleRate);
    coder.internal.errorIf(hasSampleRate && ~coder.target('MATLAB') && ...
        (~coder.internal.isConst(nrb) || ~coder.internal.isConst(scs) || ...
        ~coder.internal.isConst(nvpairs.Nfft) || ~coder.internal.isConst(nvpairs.SampleRate)), ...
        'nr5g:nrPerfectChannelEstimate:NonConstantNfftScsSampleRateNrb');
        
    ofdminfo = nr5g.internal.OFDMInfo(nrb,scs,ecp,nvpairs);

    % Validate or get default for timing offset
    if numericIdx(1)
        offset = varargin{numericIdx(1)};
        validateTimingOffset(offset,pathFilters);
    else
        % Get default
        offset = channelDelay(pathGains,pathFilters.');
    end

    % Validate or get default for sample times
    sampleTimesDefaulted = ~numericIdx(2);
    if numericIdx(2)
        sampleTimes = varargin{numericIdx(2)};
        validateSampleTimes(sampleTimes,pathGains)
    else
        % Default: vector of times at the OFDM sampling rate, one for each
        % channel snapshot in 'pathGains' and starting at zero
        sampleTimes = (0:(size(pathGains,1)-1)).' / ofdminfo.SampleRate;
    ...
    [pathFilters,offset] = adjust(pathFilters,offset,ofdminfo.CyclicPrefixLengths,ofdminfo.Nfft);
    
    end

function [pathFilters,offset] = adjust(pathFilters,offset,cpl,nfft)
    ...
    pathFilters = pathFilters(1:min(end,nfft),:);
end

% Various validation functions
function validateTimingOffset(offset,pathFilters)
    mustBeNumeric(offset);
    coder.internal.errorIf(~isscalar(offset), ...
        'CatalogID','MATLAB:system:inputMustBeScalar', ...
        'ReportedID',['MATLAB:' mfilename ':expectedScalar'],'TOFFSET')
    mustBeInteger(offset);
    mustBeNonnegative(offset);
    Nh = size(pathFilters,1);
...
function validateSampleTimes(sampleTimes,pathGains)
    if ~isempty(sampleTimes)
        validateattributes(sampleTimes,{'double'}, ...
            {'column','increasing'},mfilename,'SAMPLETIMES');
...
function validateNfftOrSampleRate(x,arg)
    if ~isempty(x)
        mustBeNumeric(x)
        coder.internal.errorIf(~isscalar(x), ...
            'CatalogID','MATLAB:system:inputMustBeScalar', ...
            'ReportedID',['MATLAB:' mfilename ':expectedScalar'],arg)        
        mustBeInteger(x)
        mustBePositive(x)
    end
end
