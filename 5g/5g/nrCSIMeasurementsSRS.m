function [CSI,CSIInfo] = nrCSIMeasurementsSRS(carrier,srs,pusch,Hest,nVar,mcsTable,bandSize)
% nrCSIMeasurementsSRS provides rank, transmit precoding matrix indicator(TPMI) and index of
% modulation and coding scheme using SRS for PUSCH transmission
%
%   [CSI,CSIINFO] = nrCSIMeasurementsSRS(CARRIER,SRS,PUSCH,HEST,NVAR,MCSTABLE,BANDSIZE)
...
if ~matlab.internal.feature('nrCSIReportCSIRS')
    % Throw error
    errorMsg = string(message("nr5g:nrCSIMeasurementsSRS:UndefinedFunction"));
    throwAsCaller(MException('MATLAB:UndefinedFunction',errorMsg));
end

% Validate input arguments
fcnName = 'nrCSIMeasurementsSRS';
% Validate 'Hest'
K = carrier.NSizeGrid;
...
if (nVar ~= 0 && ~isempty(srsIndices))
    ...
    [~,Rank] = max(efficiency);
    CSI.Rank = Rank;
    CSI.TPMI = pmiArray(:,Rank);
    CSI.MCSIndex = mcsUL{Rank};
    ...
    CSI.TPMI = NaN;
    CSI.MCSIndex = NaN;
    CSIInfo = struct('SINR',[],'SubbandIndices',[]);
end
end
...
coder.internal.errorIf(isempty(MCSTable),'nr5g:nrCSIMeasurementsSRS:InvalidMCSTable');
