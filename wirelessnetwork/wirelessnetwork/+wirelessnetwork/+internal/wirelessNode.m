classdef (Abstract) wirelessNode < handle & comm.internal.ConfigBase
    ...
    methods
        ...
        function addMobility(obj, varargin)
            %addMobility Add mobility model to wireless node
            %
            %   addMobility(OBJ, Name=Value) adds a mobility model to a wireless node, OBJ.
            %   The OBJ can be bluetoothLENode, bluetoothNode, wlanNode, or nrUE. The
            %   addMobility function sets the mobility configuration parameters using one or
            %   more optional name-value arguments. When you do not specify a name-value
            %   argument corresponding to a configuration parameter, the function assigns a
            %   default value to it. You can add a mobility model to multiple wireless nodes
            %   in a single addMobility function call, but these nodes must all use same
            %   mobility parameter values specified in the name-value arguments. To set the
            %   mobility parameters, use these name-value arguments.
            %
            %   MobilityModel - Mobility model, specified as "RandomWaypoint" or a
            %                   user-implemented custom mobility which is an object of
            %                   subclass of wirelessnetwork.internal.mobilityModel. The
            %                   default value is "RandomWaypoint". In the random waypoint
            %                   model, a wireless node starts by pausing for some duration
            %                   at a location before moving towards its next random
            %                   destination (waypoint) with a random speed. The node repeats
            %                   this process at each waypoint. When you specify a custom
            %                   mobility model, the position and velocity parameters of the
            %                   mobility model determines the initial position and the
            %                   initial velocity of the node.
            ...
            customMobilityModel = mobilityParam.MobilityModel;
            isCustomMobilityEnabled = isa(customMobilityModel, 'wirelessnetwork.internal.mobilityModel');
            % Add mobility model to each node in the given array of nodes
            for idx=1:numel(obj)
                node = obj(idx);
                if isCustomMobilityEnabled
                    % For nodes after the first one, use a copy of the custom mobility model
                    if idx > 1
                        node.Mobility = copy(customMobilityModel);
                    else
                        % For the first node, directly assign the custom mobility model
                        node.Mobility = customMobilityModel;
                    end
                    % Update the node's position property with the position value from mobility
                    % model
                    node.Position = customMobilityModel.LatestPosition;
                    % Update the node's velocity property with the velocity value from mobility
                    % model
                    node.Velocity = customMobilityModel.LatestVelocity;
                else
    ...
    methods (Access = private)
        function mobilityParam = validateMobilityInputs(obj, nvPairs)
            ...
            mobilityParam = struct("SpeedRange", [0.415 1.66], "PauseDuration", 0, "BoundaryShape", "rectangle", ...
                "Bounds", [], "MobilityModel", "RandomWaypoint");
                ...
                switch name
                    ...
                    case "MobilityModel"
                        if ~((((isstring(value) && isscalar(value)) || ischar(value)) && ...
                                strcmpi(value,"RandomWaypoint")) || ...
                                (isa(value, 'wirelessnetwork.internal.mobilityModel') && isscalar(value)))
                            error(message("wirelessnetwork:wirelessNode:InvalidMobilityObject"));
                        end
                    otherwise
