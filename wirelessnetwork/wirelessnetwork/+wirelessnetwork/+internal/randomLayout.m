function [bsCoordinates, ueCoordinates, ueAssociationInfo] = randomLayout(floorLength, floorBreadth, numBS, cellRadius, uePlacementType, numUE, propArgs)
%randomLayout Calculate base station (BS) and user equipment (UE)
%positions for the random layout scenario
%
%   Note: This is an internal undocumented method and its API and/or
%   functionality may change in subsequent releases.
%
%   [BSCOORDINATES, UECOORDINATES, UEASSOCIATIONINFO] =
%   randomLayout(NUMBS, CELLRADIUS, UEPLACEMENTTYPE, NUMUE) returns the BS
%   and UE positions, and UE association info for the given random layout
%   configuration.
%
%   BSCOORDINATES is a matrix of size N-by-3, where N is the total number
%   of BS. The 'i'th row of BSCOORDINATES contains the Cartesian coordinates
%   of a BS.
...
%   NUMBS               - Number of BS(s) in the network
...
%                         of UEs in the scenario is numBS*numUE.
...
%
%   [BSCOORDINATES, UECOORDINATES, UEASSOCIATIONINFO] =
%   randomLayout(NUMBS, CELLRADIUS, UEPLACEMENTTYPE, NUMUE, BSCOORDINATES,
%   BSID) creates additional UE(s) in an existing random layout depending
%   upon the UEPLACEMENTTYPE. If UEPLACEMENTTYPE is 0 or 1, specify all the
%   existing BS coordinates and their corresponding IDs. If UEPLACEMENTTYPE
%   is 2, specify the coordinates and ID of an existing BS, to which the
...
if (length <= 2*cellRadius)
    error("'Cell Radius' value must be less than half the length and breadth of the scenario canvas.");
end
...
if isfield(propArgs,'BSPosition') &&  isfield(propArgs,'BSId')
    ...
    bsCoordinates(:, 1) = round((2*rand(numBS, 1) - 1) * (floorLength/2 - cellRadius));
    bsCoordinates(:, 2) = round((2*rand(numBS, 1) - 1) * (floorBreadth/2 - cellRadius));
    bsID = 1:numBS;
end
...
if uePlacementType==0 % If UEs are randomly placed across the floor
    ...
    for ueIdx=1:numUE
        ...
        bsCoord = bsCoordinates(randBSIndex,:); % 3D BS coordinates
        [pos, ueAssociation] = uePlacementPerCell(bsCoord, selectedbsID, cellRadius, 1);
        ueCoordinates(ueIdx,:) = pos;
        ueAssociationInfo(ueIdx) = ueAssociation;
    end
...
% BS,UE antenna height (m) as per 3GPP TR 38.901 Table 7.4.1-1 for urban macro
bsCoordinates(:,3) = 25;
ueCoordinates(:, 3) = 1.5;
end
...
%   BSID, CELLRADIUS, NUMUE) returns UE positions for the given random
%   layout scenario.
%   
